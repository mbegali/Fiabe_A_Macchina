...
def search_online_exercise(sentence):
    query = f"Italian grammar exercise for '{sentence}' site:italiangrammar.com OR site:learnitalian.net"
    results = search(query, num_results=3, advanced=True)  # Get top 3 results
    return [result.url for result in results]  # Extract URLs from SearchResult objects
