# questo aggiunge funzione di ricerca in google
import openai
import os
import pandas as pd
...
# Function to find online grammar exercises
from googlesearch import search

# Function to find online grammar exercises on specific websites
from googlesearch import search
...
# Function to find online grammar exercises ONLY from allowed websites
def search_online_exercise(sentence):
    allowed_sites = ["oneworlditaliano.com", "impariamoitaliano.com", "learnamo.com"]
    
    # Perform a broad search (without `site:` restriction)
    query = f"Italian grammar exercise for '{sentence}'"
    results = search(query, num_results=10, advanced=True)  # Get more results to filter
    
    # Manually filter results to keep only the allowed sites
    filtered_results = [result.url for result in results if any(site in result.url for site in allowed_sites)]
    
    return filtered_results[:3] if filtered_results else ["No relevant exercises found."]
...
print("Grammar feedback added with Excel + GPT + Online Exercises!")
